...
function latestGovtShare(sharePattern) {
  if (!Array.isArray(sharePattern) || sharePattern.length === 0) return '';
  const latest = sharePattern[sharePattern.length - 1];
  const government = latest?.pattern?.government;
  if (government === null || government === undefined || government === '$undefined') return '';
  return `${government}%`;
}
...
async function scrapeCompanyDetails(symbol) {
  try {
    ...
    const range = Math.round((Number(rangeHigh) - Number(rangeLow)) * 100) / 100;

    return {
      CompanyName: company.name || '',
      GovtShare: latestGovtShare(company.sharePattern),
      Range52Wk: {
...
async function scrapeCategory() {
  try {
    const byCode = new Map(
      companies
        .filter((company) => company.sector === 'Bank')
    ...
    console.log(`Found ${priceRows.length} rows for Bank`);
